...
# or declare the whole set of networks in xng.yml and converge to it
...
copying `xng.yml` (commit it wherever you keep your infra) and applying:
...
Panel actions rewrite `xng.yml` through the YAML serializer, so comments
...
change in the page is an edit to `xng.yml` followed by `xng apply --only
...
pnpm xng panel            # listens on 127.0.0.1:7777, edits ./xng.yml
```
