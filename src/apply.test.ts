...
async function workspaceFor(yml: string): Promise<string> {
...
test('planState: converges in place, creates, removes, and refuses rather than resets', async () => {
  const ws = await workspaceFor(YML);
  const plans = await planState(parseState(YML), ws);
...
test('planState: --only removes an undeclared network but leaves the others alone', async () => {
  const ws = await workspaceFor(YML);
  const plans = await planState(parseState(YML), ws, 'old');
