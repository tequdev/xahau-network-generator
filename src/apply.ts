...
// (delete it from xng.yml, apply, add it back).
  ...
  for (const name of [...names].sort()) {
    ...
    if (!have) {
      ...
      if (diff.incompatible.length > 0) {
        ...
        plan.reason = `${diff.incompatible.join(', ')} changed; these are fixed at create time - remove the network from xng.yml, apply, then add it back`;
      } else {
