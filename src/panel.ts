...
// Every toml-editing handler must call this BEFORE touching the file: a
// request refused for a running job must not have already rewritten
// xng.toml underneath that job (or the operator's raw-editor save).
...
// start/stop/remove/upgrade are all "edit xng.toml, then `xng apply --only
...
// undeclared (removed from xng.toml but not yet applied).
...
async function route(
  ...
  if (method === 'GET' && path === '/api/state') {
    let toml = '';
    try {
      toml = await readFile(opts.file, 'utf8');
    } catch (err) {
      if ((err as NodeJS.ErrnoException).code !== 'ENOENT') throw err;
    }
    return sendJson(res, 200, { toml });
  }
  ...
  if (method === 'POST') {
    ...
    if (path === '/api/state') {
      ...
      if (typeof toml !== 'string') {
        return sendJson(res, 400, { error: 'toml must be a string' });
      }
      try {
        parseState(toml);
      } catch (err) {
      ...
      await writeFile(opts.file, toml);
      return enqueue(
        res,
        '*',
