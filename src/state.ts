...
// xng.yml: the declared set of networks. `xng apply` (and every panel
...
// A typo (`validatorz`, `network:` for `networks:`) must not silently become a default
...
function fail(where: string, msg: string): never {
  throw new Error(`xng.yml: ${where}: ${msg}`);
}
...
export function parseState(text: string): State {
  ...
  try {
    ...
    throw new Error(`xng.yml: ${(err as Error).message}`);
  }
  // An empty file is an empty declaration; anything but a mapping is not.
  ...
  if (typeof networks !== 'object' || Array.isArray(networks)) {
    fail('networks', 'must be a mapping');
  }
  let roots = 0;
...
// defaults as the CLI flags, so a network created from xng.yml and one
